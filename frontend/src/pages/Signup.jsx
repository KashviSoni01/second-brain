...
export default function Signup() {
    ...
    const handleSignup = async () => {
        try {
            setError(null);
            setLoading(true);
            const res = await fetch("https://second-brain-4-79no.onrender.com/api/auth/signup", {
                method: "POST",
                headers: {
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-hidden flex items-center justify-center px-6">
      ...
      <div className="group relative border border-slate-800/50 rounded-2xl shadow-2xl w-[460px] min-h-[660px] bg-gradient-to-br from-slate-900/70 to-slate-900/60 backdrop-blur-xl flex flex-col items-center space-y-6 md:space-y-7 px-9 py-9 transition-transform duration-300 hover:scale-[1.02] hover:border-purple-500/30 hover:shadow-purple-500/15 overflow-hidden ring-1 ring-slate-700/50">
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="relative">
            ...
            <input
              ...
              minLength={3}
              maxLength={8}
              className="w-full pl-10 pr-4 py-3 rounded-lg bg-slate-950/50 border border-slate-800/50 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300"
            />
          </div>
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="relative">
            ...
            <input
              ...
              minLength={3}
              className="w-full pl-10 pr-16 py-3 rounded-lg bg-slate-950/50 border border-slate-800/50 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300"
            />
            <button
              type="button"
        ...
        <p className="text-xs text-slate-400">
          ...
          <span onClick={() => navigate('/login')} className="ml-1 font-semibold text-purple-300 cursor-pointer hover:text-purple-200 hover:underline transition-all duration-300">
            Login
          </span>
        </p>
